...
// event into a closure nobody reads, and subscribers (`crusher run`)
...
// publish a RunComplete for its RunID, hanging a `crusher run` caller that
...
// canceled RunComplete. A canceled prompt without a RunID is dropped
// silently as before.
...
// requireSingleCanceledRunComplete reads exactly one RunComplete from ch,
// asserts it is the canceled terminal event for runID, and verifies no
// second event arrives. This observes the published pubsub event rather
// than internal bookkeeping, which is the contract a `crusher run` caller
// blocking on the broker actually relies on.
func requireSingleCanceledRunComplete(t *testing.T, ch <-chan pubsub.Event[notify.RunComplete], sessionID, runID string) {
	t.Helper()
	select {
	case ev := <-ch:
		...
		require.True(t, ev.Payload.Canceled,
			"a dropped queued prompt must publish a canceled RunComplete")
	case <-time.After(5 * time.Second):
		t.Fatal("timed out waiting for the canceled RunComplete")
	}
	select {
	case extra := <-ch:
...
// TestCancel_QueuedRunIDPromptPublishesCanceledRunComplete proves the
...
// must emit exactly one canceled RunComplete on the broker for its
...
// bookkeeping slice, not the published event a `crusher run` caller awaits.
func TestCancel_QueuedRunIDPromptPublishesCanceledRunComplete(t *testing.T) {
	t.Parallel()
	...
	requireSingleCanceledRunComplete(t, ch, sessionID, "run-queued")

	_, ok := a.messageQueue.Get(sessionID)
	require.False(t, ok, "Cancel must clear the queue")
}

// TestDrainQueueForStep_DroppedRunIDPublishesCanceledRunComplete drives
...
// canceled RunComplete on the broker. The companion bookkeeping test
// covers the returned slice; this one covers the observable terminal
// event.
func TestDrainQueueForStep_DroppedRunIDPublishesCanceledRunComplete(t *testing.T) {
	t.Parallel()
	...
	requireSingleCanceledRunComplete(t, ch, sessionID, "run-dropped")
}
