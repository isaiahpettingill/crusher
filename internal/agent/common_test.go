...
func hyperBuilder(model string) builderFunc {
	return func(t *testing.T, r *vcr.Recorder) (fantasy.LanguageModel, error) {
		provider, err := openaicompat.New(
			...
			openaicompat.WithAPIKey(os.Getenv("CRUSHER_HYPER_API_KEY")),
...
func testEnv(t *testing.T) fakeEnv {
	workingDir := filepath.Join("/tmp/crusher-test/", t.Name())
...
func coderAgent(r *vcr.Recorder, env fakeEnv, large, small fantasy.LanguageModel) (SessionAgent, error) {
	...
	// independently of user config on `$HOME/.config/crusher/crusher.json`.
	...
	cfg.Config().Options.DisabledSkills = []string{"crusher-config"}
