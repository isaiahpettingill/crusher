...
func TestGrepWithIgnoreFiles(t *testing.T) {
	...
	crusherignoreContent := "node_modules/\n"
	require.NoError(t, os.WriteFile(filepath.Join(tempDir, ".crusherignore"), []byte(crusherignoreContent), 0o644))
	...
	for name, fn := range map[string]func(pattern, path, include string) ([]grepMatch, error){
		...
		t.Run(name, func(t *testing.T) {
			...
			require.False(t, foundFiles["lib.js"], "Should not find lib.js (ignored by .crusherignore)")
...
func TestSearchImplementations(t *testing.T) {
	...
	require.NoError(t, os.WriteFile(filepath.Join(tempDir, ".crusherignore"), []byte("file5.txt\n"), 0o644))
