...
func TestReadBuiltinFile(t *testing.T) {
	...
	t.Run("reads crusher-config skill", func(t *testing.T) {
		t.Parallel()

		resp, err := readBuiltinFile(ViewParams{
			FilePath: "crusher://skills/crusher-config/SKILL.md",
		}, nil)
		require.NoError(t, err)
		require.NotEmpty(t, resp.Content)
		require.Contains(t, resp.Content, "Crusher Configuration")
	})
	...
	t.Run("not found", func(t *testing.T) {
		...
		resp, err := readBuiltinFile(ViewParams{
			FilePath: "crusher://skills/nonexistent/SKILL.md",
		}, nil)
		require.NoError(t, err)
		require.True(t, resp.IsError)
	})
	...
	t.Run("metadata has skill info", func(t *testing.T) {
		...
		require.Equal(t, "crusher-config", meta.ResourceName)
