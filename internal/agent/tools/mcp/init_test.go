...
func TestMCPSession_CancelOnClose(t *testing.T) {
	...
	client := mcp.NewClient(&mcp.Implementation{Name: "crusher-test"}, nil)
	clientSession, err := client.Connect(ctx, clientTransport, nil)
	require.NoError(t, err)

	sess := &ClientSession{clientSession, cancel}

	// Verify the context is not canceled before close.
	require.NoError(t, ctx.Err())

	err = sess.Close()
	require.NoError(t, err)

	// After Close, the context must be canceled.
...
// map so crusher_info and the TUI's MCP status card can render a real
