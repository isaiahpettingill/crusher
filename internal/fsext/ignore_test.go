...
func TestCrusherIgnore(t *testing.T) {
	tempDir := t.TempDir()
	t.Chdir(tempDir)
	...
	// Create a .crusherignore file that ignores .log files
	require.NoError(t, os.WriteFile(".crusherignore", []byte("*.log\n"), 0o644))
...
func TestShouldExcludeFile(t *testing.T) {
	...
	// Create .crusherignore file
	crusherignoreContent := "custom_ignored/\n"
	if err := os.WriteFile(filepath.Join(tempDir, ".crusherignore"), []byte(crusherignoreContent), 0o644); err != nil {
		t.Fatalf("Failed to create .crusherignore: %v", err)
	}
	...
	require.True(t, ShouldExcludeFile(tempDir, customIgnored), "Expected custom_ignored to be ignored by .crusherignore")
...
func TestShouldExcludeFileHierarchical(t *testing.T) {
	...
	// Create .crusherignore in subdir that ignores normal_nested
	subCrusherignore := "normal_nested/\n"
	if err := os.WriteFile(filepath.Join(subDir, ".crusherignore"), []byte(subCrusherignore), 0o644); err != nil {
		t.Fatalf("Failed to create subdir .crusherignore: %v", err)
	}

	// Test hierarchical ignore behavior - this should work because the .crusherignore is in the parent directory
	require.True(t, ShouldExcludeFile(tempDir, nestedNormal), "Expected normal_nested to be ignored by subdir .crusherignore")
