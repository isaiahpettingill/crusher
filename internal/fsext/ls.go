...
// crusherGlobalIgnorePatterns returns patterns from the user's
// ~/.config/crusher/ignore file.
var crusherGlobalIgnorePatterns = sync.OnceValue(func() []gitignore.Pattern {
	name := filepath.Join(home.Config(), "crusher", "ignore")
	bts, err := os.ReadFile(name)
	if err != nil {
		if !os.IsNotExist(err) {
			slog.Debug("Failed to read crusher global ignore file", "path", name, "error", err)
		}
		return nil
	}
...
type directoryLister struct {
	// dirPatterns caches parsed patterns from .gitignore/.crusherignore for each directory.
...
// .gitignore and .crusherignore files. Results are cached.
func (dl *directoryLister) getDirPatterns(dir string) []gitignore.Pattern {
	return dl.dirPatterns.GetOrSet(dir, func() []gitignore.Pattern {
		...
		for _, ignoreFile := range []string{".gitignore", ".crusherignore"} {
...
func (dl *directoryLister) getCombinedMatcher(dir string) gitignore.Matcher {
	return dl.combinedMatchers.GetOrSet(dir, func() gitignore.Matcher {
		...
		// Add global ignore patterns (git core.excludesFile + crusher global ignore).
		allPatterns = append(allPatterns, gitGlobalIgnorePatterns()...)
		allPatterns = append(allPatterns, crusherGlobalIgnorePatterns()...)
