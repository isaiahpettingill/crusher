...
// completion contract (notably `crusher run` in client/server mode)
...
// with an error; Canceled is true when terminated due to context
// cancellation.
//
...
// `crusher run`, which may attach to a busy session whose currently
