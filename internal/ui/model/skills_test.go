...
func TestSkillStatusItemsExcludesDisabledSkills(t *testing.T) {
	...
	ui := &UI{
		com: &common.Common{
			Styles:    &st,
			Workspace: &testWorkspace{cfg: &config.Config{Options: &config.Options{DisabledSkills: []string{"go-doc", "crusher-config"}}}},
		},
		skillStates: []*skills.SkillState{
			{Name: "go-doc", Path: "/tmp/go-doc/SKILL.md", State: skills.StateNormal},
		},
	...
	for _, item := range items {
		...
		require.NotEqual(t, "crusher-config", item.name)
	}
}
