...
func TestToPromptXMLBuiltinType(t *testing.T) {
	...
	skills := []*Skill{
		{Name: "builtin-skill", Description: "A builtin.", SkillFilePath: "crusher://skills/builtin-skill/SKILL.md", Builtin: true},
		{Name: "user-skill", Description: "A user skill.", SkillFilePath: "/home/user/.config/crusher/skills/user-skill/SKILL.md"},
	}
	xml := ToPromptXML(skills)
...
func TestDiscoverBuiltin(t *testing.T) {
	...
	for _, s := range discovered {
		if s.Name == "crusher-config" {
			found = true
			require.True(t, strings.HasPrefix(s.SkillFilePath, BuiltinPrefix))
			require.True(t, strings.HasPrefix(s.Path, BuiltinPrefix))
			require.Equal(t, "crusher://skills/crusher-config/SKILL.md", s.SkillFilePath)
			require.Equal(t, "crusher://skills/crusher-config", s.Path)
			require.NotEmpty(t, s.Description)
			require.NotEmpty(t, s.Instructions)
			require.True(t, s.Builtin)
		}
	}
	require.True(t, found, "crusher-config builtin skill not found")

	var foundJQ bool
	for _, s := range discovered {
		if s.Name == "jq" {
			foundJQ = true
			require.Equal(t, "crusher://skills/jq/SKILL.md", s.SkillFilePath)
			require.Equal(t, "crusher://skills/jq", s.Path)
			require.NotEmpty(t, s.Description)
			require.NotEmpty(t, s.Instructions)
			require.True(t, s.Builtin)
		}
	...
	var foundHooks bool
	for _, s := range discovered {
		if s.Name == "crusher-hooks" {
			foundHooks = true
			require.Equal(t, "crusher://skills/crusher-hooks/SKILL.md", s.SkillFilePath)
			require.Equal(t, "crusher://skills/crusher-hooks", s.Path)
			require.NotEmpty(t, s.Description)
			require.NotEmpty(t, s.Instructions)
			require.True(t, s.Builtin)
		}
	}
	require.True(t, foundHooks, "crusher-hooks builtin skill not found")
}
...
func TestDeduplicate(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			input:    []*Skill{{Name: "crusher-config", Path: "crusher://skills/crusher-config"}, {Name: "crusher-config", Path: "/user/crusher-config"}},
			wantLen:  1,
			wantName: "crusher-config",
			wantPath: "/user/crusher-config",
		},
		{
			name:    "empty",
			input:   nil,
