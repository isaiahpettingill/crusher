---
name: crusher-config
description: Use when the user needs help configuring Crusher — working with crusher.json, setting up providers, configuring LSPs, adding MCP servers, managing skills or permissions, or changing Crusher behavior.
---

# Crusher Configuration

Crusher uses JSON configuration files with the following priority (highest to lowest):

1. `.crusher.json` (project-local, hidden)
2. `crusher.json` (project-local)
3. `$XDG_CONFIG_HOME/crusher/crusher.json` or `$HOME/.config/crusher/crusher.json` (global)
...
Crusher runs selected string fields through an embedded bash-compatible
...
`crusher.json` is trusted code. Any `$(...)` in it runs at load time
with the invoking user's shell privileges, before the UI appears.
Don't launch Crusher in a directory whose `crusher.json` you haven't
reviewed.
...
> `.agents/skills`, `.crusher/skills`, `.claude/skills`, `.cursor/skills`
    ...
    "PreToolUse": [
      {
        ...
        "command": ".crusher/hooks/protect-files.sh"
      },
      {
        "matcher": "^bash$",
        "command": ".crusher/hooks/no-haskell.sh"
      }
    ]
  }
}
...
|---|---|
| `CRUSHER_EVENT` | Event name (e.g. `PreToolUse`) |
| `CRUSHER_TOOL_NAME` | Name of the tool being called |
| `CRUSHER_SESSION_ID` | Current session ID |
| `CRUSHER_CWD` | Current working directory |
| `CRUSHER_PROJECT_DIR` | Project root directory |
| `CRUSHER_TOOL_INPUT_COMMAND` | Value of `command` from tool input (if present) |
| `CRUSHER_TOOL_INPUT_FILE_PATH` | Value of `file_path` from tool input (if present) |
...
Crusher also supports the Claude Code hook output format:
...
- `CRUSHER_GLOBAL_CONFIG` - Override global config location
- `CRUSHER_GLOBAL_DATA` - Override data directory location
- `CRUSHER_SKILLS_DIR` - Override default skills directory
