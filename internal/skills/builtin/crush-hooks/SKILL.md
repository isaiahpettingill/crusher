---
name: crusher-hooks
description: Use when the user wants to add, write, debug, or configure a Crusher hook — gating or blocking tool calls, approving or rewriting tool input before execution, injecting context into tool results, or troubleshooting hook behavior in crusher.json.
---

# Crusher Hooks

Hooks are user-defined commands in `crusher.json` that fire at specific points
...
3. Add the entry under `hooks.PreToolUse` in `crusher.json` with the right matcher.
...
- Non-zero exit codes other than 2/49 are logged but don't block — check Crusher logs.
...
Crusher also accepts Claude Code's `hookSpecificOutput` envelope. One intentional
divergence: Crusher treats `updated_input` as shallow-merge, Claude Code replaces.
