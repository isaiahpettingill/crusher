...
func TestTracker_BuiltinSkillTracking(t *testing.T) {
	...
	// Simulate active skills including a builtin skill (crusher-config).
	activeSkills := []*Skill{
		{Name: "crusher-config", Description: "Crusher config", Builtin: true},
		{Name: "go-doc", Description: "Go docs", Builtin: false},
	}
	tracker := NewTracker(activeSkills)

	// Initially not loaded.
	require.False(t, tracker.IsLoaded("crusher-config"))
	require.False(t, tracker.IsLoaded("go-doc"))

	// Mark builtin skill as loaded (simulating read via crusher://...).
	tracker.MarkLoaded("crusher-config")
	require.True(t, tracker.IsLoaded("crusher-config"))
