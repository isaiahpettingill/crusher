...
import (
	...
	_ "github.com/isaiahpettingill/crusher/internal/swagger"
...
// socketDir returns the directory used for the Crusher Unix socket.
...
// returned by [socketDir] and is named crusher-<uid>.sock, falling
// back to crusher.sock when the current uid cannot be determined. If
// the composed path would exceed [maxUnixSocketPathLen] bytes (the
// macOS sun_path limit), we fall back to /tmp/crusher-<uid>.sock so
// the socket remains bindable.
func DefaultHost() string {
	sock := "crusher.sock"
	usr, err := user.Current()
	if err == nil && usr.Uid != "" {
		sock = fmt.Sprintf("crusher-%s.sock", usr.Uid)
	}
...
// Server represents a Crusher server bound to a specific address.
type Server struct {
