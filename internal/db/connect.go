...
// crusher process from racing the storage.
...
// workspace-bootstrap path opts in. CRUSHER_SKIP_DATADIR_LOCK still
...
func Connect(ctx context.Context, dataDir string, opts ...ConnectOption) (*sql.DB, error) {
	...
	dbPath := filepath.Join(dataDir, "crusher.db")
	...
	// crusher process on the same SQLite file. The lock is released when
	...
	// local-mode invocations do not refuse a second crusher against the
