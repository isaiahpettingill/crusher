...
// already in use by another crusher process.
var ErrDataDirLocked = errors.New("data directory already in use by another crusher process")

// dataDirLockFile is the name of the lock file inside the data
// directory. It lives next to crusher.db so users can `ls` and find it.
const dataDirLockFile = "crusher.lock"
...
// {dataDir}/crusher.lock. If the lock is already held by another
...
// CRUSHER_SKIP_DATADIR_LOCK is set to a truthy value. This is intended
...
func skipDataDirLock() bool {
	v, _ := strconv.ParseBool(os.Getenv("CRUSHER_SKIP_DATADIR_LOCK"))
	return v
}
