...
func TestConfigStore_ConfigPath_GlobalAlwaysWorks(t *testing.T) {
	...
	store := &ConfigStore{
		globalDataPath: "/some/global/crusher.json",
	}

	path, err := store.configPath(ScopeGlobal)
	require.NoError(t, err)
	require.Equal(t, "/some/global/crusher.json", path)
}
...
func TestConfigStore_ConfigPath_WorkspaceReturnsPath(t *testing.T) {
	...
	store := &ConfigStore{
		workspacePath: "/some/workspace/.crusher/crusher.json",
	}

	path, err := store.configPath(ScopeWorkspace)
	require.NoError(t, err)
	require.Equal(t, "/some/workspace/.crusher/crusher.json", path)
}
...
func TestGlobalWorkspaceDir(t *testing.T) {
	dir := t.TempDir()
	t.Setenv("CRUSHER_GLOBAL_DATA", dir)
...
func TestReloadFromDisk_UsesNewConfigValues(t *testing.T) {
	...
	t.Setenv("CRUSHER_GLOBAL_CONFIG", dir)
	t.Setenv("CRUSHER_GLOBAL_DATA", dir)
	resetProviderState()
