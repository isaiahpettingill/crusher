...
func TestBuildEnv(t *testing.T) {
	...
	require.Equal(t, EventPreToolUse, envMap["CRUSHER_EVENT"])
	require.Equal(t, "bash", envMap["CRUSHER_TOOL_NAME"])
	require.Equal(t, "sess-1", envMap["CRUSHER_SESSION_ID"])
	require.Equal(t, "/work", envMap["CRUSHER_CWD"])
	require.Equal(t, "/project", envMap["CRUSHER_PROJECT_DIR"])
	require.Equal(t, "ls", envMap["CRUSHER_TOOL_INPUT_COMMAND"])
	require.Equal(t, "/tmp/f.txt", envMap["CRUSHER_TOOL_INPUT_FILE_PATH"])

	// Shared Crusher markers must be present so hook-authored scripts can
	// detect they're running under Crusher the same way bash-tool-invoked
	// scripts can.
	require.Equal(t, "1", envMap["CRUSHER"])
	require.Equal(t, "crusher", envMap["AGENT"])
	require.Equal(t, "crusher", envMap["AI_AGENT"])
}
...
func TestRunnerEnvVarsPropagated(t *testing.T) {
	t.Parallel()
	hookCfg := config.HookConfig{
		Command: `printf '{"decision":"allow","context":"%s"}' "$CRUSHER_TOOL_NAME"`,
	}
	r := NewRunner([]config.HookConfig{hookCfg}, t.TempDir(), t.TempDir())
	result, err := r.Run(context.Background(), EventPreToolUse, "sess", "bash", `{}`)
	require.NoError(t, err)
...
func TestParseStdoutClaudeCodeFormat(t *testing.T) {
	...
	t.Run("crusher format still works", func(t *testing.T) {
		t.Parallel()
