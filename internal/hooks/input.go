...
func BuildEnv(eventName, toolName, sessionID, cwd, projectDir, toolInputJSON string) []string {
	env := os.Environ()
	env = append(env, shell.CrusherEnvMarkers()...)
	env = append(
		env,
		fmt.Sprintf("CRUSHER_EVENT=%s", eventName),
		fmt.Sprintf("CRUSHER_TOOL_NAME=%s", toolName),
		fmt.Sprintf("CRUSHER_SESSION_ID=%s", sessionID),
		fmt.Sprintf("CRUSHER_CWD=%s", cwd),
		fmt.Sprintf("CRUSHER_PROJECT_DIR=%s", projectDir),
	)

	// Extract tool-specific env vars from the JSON input.
	if toolInputJSON != "" {
		if cmd := gjson.Get(toolInputJSON, "command"); cmd.Exists() {
			env = append(env, fmt.Sprintf("CRUSHER_TOOL_INPUT_COMMAND=%s", cmd.String()))
		}
		if fp := gjson.Get(toolInputJSON, "file_path"); fp.Exists() {
			env = append(env, fmt.Sprintf("CRUSHER_TOOL_INPUT_FILE_PATH=%s", fp.String()))
		}
	}
...
// Supports both Crusher format and Claude Code format (hookSpecificOutput).
