...
name: builtin-skills
description:
  Use when creating a new builtin skill for Crusher, editing an existing builtin
...
Crusher embeds skills directly into the binary via `internal/skills/builtin/`.
...
- `DiscoverBuiltin()` walks the embedded FS, parses each `SKILL.md`, and sets
  paths with the `crusher://skills/` prefix (e.g., `crusher://skills/jq/SKILL.md`).
- The View tool resolves `crusher://` paths from the embedded FS, not disk.
...
| -------------- | ----------------------- | ------------------------------------------ |
| `crusher-config` | `builtin/crusher-config/` | Crusher configuration help                   |
| `crusher-hooks`  | `builtin/crusher-hooks/`  | Authoring, configuring and debugging hooks |
