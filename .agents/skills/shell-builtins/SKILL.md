---
name: shell-builtins
description: Use when creating a new shell builtin command for Crusher (internal/shell/), editing an existing one, or when the user needs to understand how commands are intercepted in Crusher's embedded shell.
---

# Shell Builtins

Crusher's shell (`internal/shell/`) uses `mvdan.cc/sh/v3` for POSIX shell
