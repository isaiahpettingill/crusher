...
contents. Crusher tells the agent the files exist and are relevant, and the agent
...
Paths are resolved relative to `CRUSHER_CWD`. Non-existent paths are dropped with
a debug log (don't fail the hook over a missing file).

### How the agent sees it

Crusher appends a short note to the turn's context along the lines of:
...
- Should `context_files` paths be constrained to `CRUSHER_PROJECT_DIR`? Probably
...
Today Crusher supports exactly one hook event, `PreToolUse`. That's enough to gate
