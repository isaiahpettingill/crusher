...
the agent lifecycle, allowing you to both build on top of Crusher, customize
...
- Crusher ships with a builtin `crusher-hook` skill write, edit, and configure
  hooks; just tell Crusher how to configure Crusher
- Crusher currently supports just one hook, `PreToolUse`, with plans to support
...
our **project-level** `crusher.json`. Relative paths like `./no-haskell.sh` work
here because the project root is your working directory. If you're configuring
a global hook (`~/.config/crusher/crusher.json`), use an absolute path instead.
...
# ($CRUSHER_TOOL_INPUT_COMMAND) to grep and match on a regexp.
if echo "$CRUSHER_TOOL_INPUT_COMMAND" | grep -qE '(^| )((ghc|cabal|stack)(\.exe)?)( |$)'; then
...
Hooks run through Crusher's embedded POSIX shell (`mvdan.cc/sh`) — the same
...
- **Permissive shebang fallback**: if the absolute path in a shebang
  doesn't exist (e.g. `#!/bin/bash` on Windows), Crusher falls back to a
  ...
  "no opinion".
- **Environment**: every hook sees `CRUSHER=1`, `AGENT=crusher`, and
  `AI_AGENT=crusher` on top of the `CRUSHER_*` hook-specific variables. These
  ...
  both contexts.
- **Timeout behavior**: when a hook exceeds its timeout, Crusher cancels the
  context and waits a short grace period (~1s) for the interpreter to
  yield. If the hook still hasn't returned, Crusher abandons it, logs a
...
Hooks can be added to your `crusher.json` (or `.crusher.json`) at both the global
...
> work fine in project-level `crusher.json` because the project root is also
> your working directory. For **global** config (`~/.config/crusher/`),
> however, you must use either an absolute path or an inline command:
>
> ```jsonc
> // Global ~/.config/crusher/crusher.json
> {
>   "hooks": {
>     "PreToolUse": [
>       {
>         "command": "/home/you/.config/crusher/hooks/no-haskell.sh"
...
3. Runs all matching hooks **in parallel** through Crusher's embedded POSIX
...
5. Applies the result **before** permission checks. If the aggregated decision
   ...
   for it. If it's `allow`, Crusher treats that as affirmative pre-approval and
...
however you'll incur some additional overhead as Crusher will still parse and
run each hook.
...
| ---------------------------- | ---------------------------------------------- |
| `CRUSHER`                      | Always `1` when running under Crusher.           |
| `AGENT`                      | Always `crusher`.                                |
| `AI_AGENT`                   | Always `crusher`.                                |
| `CRUSHER_EVENT`                | The hook event name (e.g. `PreToolUse`).       |
| `CRUSHER_TOOL_NAME`            | The tool being called (e.g. `bash`).           |
| `CRUSHER_SESSION_ID`           | Current session ID.                            |
| `CRUSHER_CWD`                  | Working directory.                             |
| `CRUSHER_PROJECT_DIR`          | Project root directory.                        |
| `CRUSHER_TOOL_INPUT_COMMAND`   | For `bash` calls: the shell command being run. |
| `CRUSHER_TOOL_INPUT_FILE_PATH` | For file tools: the target file path.          |

The `CRUSHER`, `AGENT`, and `AI_AGENT` markers are also set by the `bash`
tool, so a script can detect "am I running under Crusher?" the same way in
either context.
...
Hooks communicate back to Crusher via **exit code** and `stdout`/`stderr`. The
...
deterministic based on the order hooks appear in `crusher.json`.
...
If a hook exceeds its timeout, Crusher cancels its context and treats the
...
if echo "$CRUSHER_TOOL_INPUT_COMMAND" | grep -qE 'rm\s+-(rf|fr)\s+/'; then
...
returns `decision: "allow"`, which tells Crusher to pre-approve the call:
...
case "$CRUSHER_TOOL_INPUT_COMMAND" in
...
if [[ "$CRUSHER_TOOL_INPUT_FILE_PATH" == *.go ]]; then
...
```jsonc
{ "command": "echo \"$(date -Iseconds) $CRUSHER_TOOL_NAME\" >> ./tools.log" }
```
...
Crusher hooks are broadly compatible with [Claude Code
...
hooks run under Crusher unchanged. This document covers the Crusher-specific API
only — anything not documented here isn't guaranteed to work.

One intentional divergence: Crusher treats `updated_input` as a shallow-merge
...
4. `decision` precedence: `deny` > `allow` > `null`. First deny determines the
   ...
   is `allow`, Crusher pre-approves the tool call and skips the permission
